...
### [ ] `sql/2026-09-lama-arqueo-eliminar.sql` — el botón "Borrar arqueo"

--  DÓNDE VA:  Supabase -> SQL Editor -> New query
--  ES:        1 solo bloque
--  TARDA:     instantáneo
--  QUÉ HACE:  deja que un arqueo se marque "eliminado". NO borra ninguna
--             fila: queda en la lista tachado, con quién y cuándo.
--  QUÉ VER:   "1 firma" en arqueo_eliminar, y "sí" en el estado nuevo.

**Sin esto, el basurero del arqueo avisa que falta este `.sql`** y no hace
nada más. Todo lo demás de la página de Arqueo funciona sin él.
...
### [ ] `sql/2026-09-lama-arqueo-horario.sql` — la fecha y hora de apertura

--  DÓNDE VA:  Supabase -> SQL Editor -> New query
--  ES:        1 solo bloque
--  TARDA:     instantáneo
--  QUÉ HACE:  deja elegir la fecha y hora de apertura (hacia atrás, nunca
--             adelante), guarda aparte la hora real en que se apretó el
--             botón, y hace que una venta ya contada en un arqueo cerrado
--             no se cuente otra vez en el siguiente. No borra nada.
--  QUÉ VER:   "1 firma" en arqueo_abrir y en arqueo_cerrar, y "0" en
--             "arqueos sin hora real".

**Se puede pegar antes o después del de borrar**: no dependen uno del otro.
Mientras no esté, la caja se abre igual con la hora de ahora; lo único que
avisa que falta es tocar la hora de apertura.

**Probado en un Postgres de verdad** armado con los `.sql` del repo en orden: caja
AM, venta cobrada en el hueco, caja PM abierta con la hora hacia atrás pisando la
AM. La PM contó 800 (los 500 del hueco más sus 300) y no los 1.000 de la AM. El
mismo caso sin la venta de 300, con la función de ayer, contaba 1.500 en vez de
500: le volvía a sumar lo de la mañana.
...
### [x] `sql/2026-09-lama-arqueo-fudo.sql` — **corrido el 2026-09-22** ✅

> **El arqueo pasa a comparar CADA medio de pago, confirmado sano:** el
> bloque 8 devolvió "1 firma" en `arqueo_cerrar`, `arqueo_declarar` y
> `arqueo_movimiento`, y las dos tablas nuevas (`lama_arqueo_declarado`,
> `lama_arqueo_cierre_medios`) en 0 filas — como corresponde a un
> `create table` recién corrido. Reemplaza el modelo de
> `sql/2026-09-lama-arqueo.sql` (que solo miraba el efectivo) sin perder
> ninguna caja ya abierta o cerrada.
