...
### [ ] `sql/2026-09-lama-arqueo.sql` — la caja del turno (F6)

--  DÓNDE VA:  Supabase -> SQL Editor -> New query
--  ES:        1 solo bloque
--  TARDA:     instantáneo
--  QUÉ HACE:  crea el arqueo de caja — abrir con un monto inicial, anotar
--             ingresos y egresos de mano, y cerrar contando el efectivo
--             contra lo que el sistema dice que debería haber. También le
--             agrega a `item_anular` el candado de que una venta de un
--             arqueo ya cerrado no se puede anular.
--  QUÉ VER:   el bloque final tiene que decir, para cada función, "1 firma".
--             Si alguna dice 2, avisame antes de usar la pantalla — no la
--             uses todavía.

**Es "una de las joyas de la corona"** — tus palabras, 2026-09-21. Ya está
**construida y publicada** en la app (el aviso arriba de Mesas, abrir/cerrar
caja, movimientos de mano). **Lo único que falta sos vos, pegando esto.**
Mientras no lo pegues, la pantalla de Mesas sigue funcionando exactamente
igual que ahora — el aviso de caja simplemente no aparece.

El interruptor del **arqueo ciego** (que el cajero cuente sin ver cuánto
debería haber) ya vive en **Ajustes → Interruptores**, apagado de fábrica —
no hace falta otro `.sql` para eso.
